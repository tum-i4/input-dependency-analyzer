#include "analysis/LibraryInfoFromConfigFile.h"
...
void LibraryInfoFromConfigFile::parse_dependencies(LibFunctionInfo& libInfo, const json& arg_deps)
{
    ...
    for (unsigned i = 0; i < arg_deps.size(); ++i) {
        ...
        for (auto it = arg_dep.begin(); it != arg_dep.end(); ++it) {
            ...
            LibFunctionInfo::LibArgDepInfo argInputDepInfo;
            if (entry_deps.dependency != InputDepInfo::UNKNOWN) {
                argInputDepInfo.dependency = entry_deps.dependency;
            } else {
                argInputDepInfo.argumentsIdx = std::move(entry_deps.argumentsIdx);
            }

            if (it.key() == "return") {
                returnDeps = argInputDepInfo;
            } else {
                try {
                    int arg_num = std::stoi(it.key());
                    argDeps.insert(std::make_pair(arg_num, argInputDepInfo));
...
{
    InputDepInfo::Dependency dep = InputDepInfo::UNKNOWN;
    ...
    for (const auto& val : entry) {
        if (val.is_number()) {
            ...
            if (val_str == "dep") {
                dep = InputDepInfo::INPUT_DEP;
            } else if (val_str == "indep") {
                dep = InputDepInfo::INPUT_INDEP;
            }
            break;
        }
    }
