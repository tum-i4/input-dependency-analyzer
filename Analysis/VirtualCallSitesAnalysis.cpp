...
class VirtualCallSitesAnalysis::Impl
{
private:
    struct VTableSlot
    {
...
VirtualCallSiteAnalysisResult& VirtualCallSitesAnalysis::Impl::getAnalysisResult()
{
    return m_results;
}


const VirtualCallSiteAnalysisResult& VirtualCallSitesAnalysis::Impl::getAnalysisResult() const
{
    return m_results;
}

void VirtualCallSitesAnalysis::Impl::runOnModule(llvm::Module& M)
{
    m_module = &M;
...
void VirtualCallSitesAnalysis::Impl::buildTypeIdentifierMap(
                                          std::vector<llvm::wholeprogramdevirt::VTableBits>& Bits,
                                          std::unordered_map<llvm::Metadata*, std::set<llvm::wholeprogramdevirt::TypeMemberInfo>>& TypeIdMap)
{
...
bool VirtualCallSitesAnalysis::Impl::tryFindVirtualCallTargets(
                                   std::vector<llvm::wholeprogramdevirt::VirtualCallTarget>& TargetsForSlot,
                                   const std::set<llvm::wholeprogramdevirt::TypeMemberInfo>& TypeMemberInfos,
                                   uint64_t ByteOffset)
{
...
void VirtualCallSitesAnalysis::Impl::updateResults(const std::vector<VirtualCallSite>& S,
                                                   const std::vector<llvm::wholeprogramdevirt::VirtualCallTarget> TargetsForSlot)
{
    for (const auto& cs : S) {
...
char VirtualCallSitesAnalysis::ID = 0;

VirtualCallSitesAnalysis::VirtualCallSitesAnalysis()
    : llvm::ModulePass(ID)
    , m_impl(new Impl())
{
}

bool VirtualCallSitesAnalysis::runOnModule(llvm::Module& M)
{
    m_impl->runOnModule(M);
}

VirtualCallSiteAnalysisResult& VirtualCallSitesAnalysis::getAnalysisResult()
{
    return m_impl->getAnalysisResult();
}

const VirtualCallSiteAnalysisResult& VirtualCallSitesAnalysis::getAnalysisResult() const
{
    return m_impl->getAnalysisResult();
}

static llvm::RegisterPass<VirtualCallSitesAnalysis> X("virtual-calls","runs virtual call analysis pass");
